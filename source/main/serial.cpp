...
/*---------------------------------------------------------------------------*/
/**
  @brief	use amode to find out the mode of the port
  @param	amode	char array with mode from profile
  @return	Serial mode, see also moxadevice.h
 ...
 */
/*---------------------------------------------------------------------------*/
int RS_STRFLOW(const char * aflow){
		 if ( strcmp(aflow, "NO_FLOW_CONTROL") == 0 ) return NO_FLOW_CONTROL;
	else if ( strcmp(aflow, "HW_FLOW_CONTROL") == 0 ) return HW_FLOW_CONTROL;
	else if ( strcmp(aflow, "SW_FLOW_CONTROL") == 0 ) return SW_FLOW_CONTROL;
	else if ( strcmp(aflow, "MY_FLOW_CONTROL") == 0 ) return MY_FLOW_CONTROL;
	else return -1;
}
/*---------------------------------------------------------------------------*/
/**
 ...
 */
/*---------------------------------------------------------------------------*/
unsigned int RS_STRMODE(const char * amode){
	if ( strcmp(amode, "RS232_MODE") == 0 ) return RS232_MODE;
	else if ( strcmp(amode, "RS422_MODE") == 0 ) return RS422_MODE;
	else if ( strcmp(amode, "RS485_2WIRE_MODE") == 0 ) return RS485_2WIRE_MODE;
	else if ( strcmp(amode, "RS485_4WIRE_MODE") == 0 ) return RS485_4WIRE_MODE;
	else return -1;
}
/*---------------------------------------------------------------------------*/
...
int	SerialFlowControl( int port, int control)
{
	...
	else if(control == MY_FLOW_CONTROL) 
		newtio[ port].c_iflag &= !(IXON | IXOFF | IXANY);
